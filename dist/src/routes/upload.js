...
router.post("/upload", upload.single("file"), async (req, res) => {
    try {
        ...
        if (file.mimetype === "text/csv") {
            ...
            const budgetInputSheet = workbook.SheetNames.find(name => name.toLowerCase().includes('budget_input_2025') ||
            ...
            if (!targetSheet) {
                ...
                const fiscalYearSheet = workbook.SheetNames.find(name => name.toLowerCase().includes('fiscal_year_23-25') ||
                ...
                if (!targetSheet) {
                    for (const sheetName of workbook.SheetNames) {
                        if (prioritySheets.some(priority => sheetName.toLowerCase().includes(priority.toLowerCase()) &&
                            ...
                            if (sheetData.length > 1) {
                                ...
                                if (sheetRows.length > 0) {
                                    ...
                                    if (altSheetData.length > 0) {
                                        ...
                                        const altRows = altSheetData.filter((row) => {
                                            return Object.values(row).some(value => value !== null && value !== undefined && value !== '' && value !== 0);
                                        });
                        ...
                        if (sheetData.length > 1) {
                            ...
                            if (sheetRows.length > 0) {
                                ...
                                const hasBudgetColumns = sheetRows.some(row => {
                                    ...
                                    return keys.some(key => key && (key.toLowerCase().includes('quarter') ||
            ...
            const budgetInputSheetName = workbook.SheetNames.find(name => name === 'Budget_Input' || name === 'Budget_input_2025' ||
                name.toLowerCase().includes('budget_input'));
        ...
        const normalizedRows = rows.map((row, index) => {
            ...
            const findQuarterValue = (quarterNum) => {
                const patterns = [
                ...
                const rowKeys = Object.keys(row);
                const quarterKey = rowKeys.find(key => key && (key.includes(`Q${quarterNum}`) ||
            ...
            const isMetadata = metadataKeywords.some(keyword => row.description && row.description.toLowerCase().includes(keyword));
            if (isMetadata) {
        ...
        const proposal = await prisma_1.default.budgetProposal.create({
            data: {
        ...
        for (const row of normalizedRows) {
            const lineItem = await prisma_1.default.budgetLineItem.create({
                data: {
            ...
            for (const alloc of allocations) {
                if (alloc.proposedAmount > 0) {
                    await prisma_1.default.budgetAllocation.create({
                        data: {
...
router.get("/rows/:userId", async (req, res) => {
    try {
        ...
        const allRows = proposals.flatMap((proposal) => proposal.lineItems.map((item) => {
