...
router.get("/generate/:userId", async (req, res) => {
    try {
        ...
        const historicalData = proposals.flatMap((proposal) => proposal.lineItems.map((item) => {
        ...
        // Generate forecast
        const forecasts = await generateForecast(historicalData, String(department), parsedSeasonalityPeriod, parsedAlpha, parsedBeta, parsedGamma);
